...
export const loginToX = async (username, password) => {
  ...
  if (!isLoggedIn) {
    ...
    let foundAndClicked = await findAndClick(
      page,
    ...
    let foundAndTyped = await findAndType(
      page,
      process.env.TWITTER_USERNAME_INPUT,
      this.botUsername
    );
    ...
    foundAndClicked = await findAndClick(
      page,
    ...
    foundAndTyped = await findAndType(
      page,
      process.env.TWITTER_PASSWORD_INPUT,
      this.botPassword
    );
    ...
    console.log(
      "Twitter Bot has logged in, we now will try to detect suspicion."
    );
    ...
    if (confirmedSuspicion) {
      ...
      console.log(
        "We will now try to see if Twitter wants verification from us."
      );
      ...
      if (confirmedVerification.success) {
        ...
        return this.respond(
          false,
          "Bot did NOT log in / Twitter wants verification code."
        );
...
const twitterSuspects = async (page) => {
  try {
    const TwitterSuspects = await page.waitForXPath(
...
const twitterWantsVerification = async (page) => {
  try {
    const TwitterWantsToVerify = await page.waitForXPath(
